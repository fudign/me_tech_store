...
must_haves:
  ...
  artifacts:
    ...
    - path: "app/Models/Product.php"
      ...
      exports: ["categories", "attributes"]
    - path: "app/Models/ProductAttribute.php"
      provides: "ProductAttribute model with product relationship"
      exports: ["product"]
  key_links:
    ...
    - from: "app/Models/Product.php"
      to: "app/Models/ProductAttribute.php"
      via: "hasMany relationship"
      pattern: "hasMany(ProductAttribute::class)"
    - from: "app/Models/ProductAttribute.php"
...
Output: Working Laravel 12 installation with Product/Category/ProductAttribute/User models, secure session configuration, and production-ready database schema.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    6. Create Product model with Sluggable:
       ...
       - ProductAttribute model with belongsTo completes the relationship chain

    7. Create ProductAttribute model with Product relationship:
       ```php
       namespace AppModels;
       ...
       CRITICAL: This model is required for Plan 01-03 to filter products by attributes.
       The belongsTo relationship completes the bidirectional link with Product::attributes().
  </action>
  <verify>
    ```bash
    # Create database
  ...
  <done>
    ...
    - ProductAttribute model created with belongsTo(Product::class) relationship
    - Product->attributes() hasMany relationship works bidirectionally with ProductAttribute->product()
...
# 5. Verify ProductAttribute model and relationship
php artisan tinker --execute="
$prod = Product::first();
$attr = $prod->attributes()->create(['key' => 'memory', 'value' => '256GB']);
echo 'Attribute product: ' . $attr->product->name;
echo 'Product attributes count: ' . $prod->attributes->count();
"
...
# 7. Verify session security settings
grep -E "(http_only|same_site)" config/session.php
...
- ProductAttribute model exists and relationships work bidirectionally
...
4. **Models**:
   - Product model with auto-slug generation, category relationship, attributes relationship
   - ProductAttribute model with product relationship (bidirectional with Product)
...
5. **Verification**:
   ...
   - Can create ProductAttribute, access via Product->attributes() and ProductAttribute->product()
